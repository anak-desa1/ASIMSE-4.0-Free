...
class Persiapan_tahun_aktif extends CI_Controller
{
    function __construct()
    {
        ...
        $this->load->model('PersiapanTahunAktif_model', 'tahun_aktif');
    }
    ...
    public function index()
    {
        if ($this->form_validation->run() == false) {
            ...
            $this->load->view('data_persiapan/persiapan_tahun_aktif/tahun_aktif_css');
            $this->load->view('layout/header-bottom', $data);
            $this->load->view('layout/main-navigation', $data);
            $this->load->view('data_persiapan/persiapan_tahun_aktif/tahun_aktif_v', $data);
            $this->load->view('layout/footer-top');
            $this->load->view('data_persiapan/persiapan_tahun_aktif/tahun_aktif_js');
    ...
    public function tampildata()
    {
        ...
        foreach ($list as $data) {
            ...
            $tombol_action = ($data['aktif'] == "N" ?
                '<form action="' . base_url('data_persiapan/persiapan_tahun_aktif/aktif')  . '" method="post" enctype="multipart/form-data">                    
                ...
                ' <form action="' . base_url('data_persiapan/persiapan_tahun_aktif/tidak_aktif') . '" method="post" enctype="multipart/form-data">                    
                        <input type="hidden" name="_id" id="_id" value="' . $data['id'] . '">
                ...
                (cek_akses_user()['role_id'] == 1 ?
                    ' <form action="' . base_url('data_persiapan/persiapan_tahun_aktif/edit/') . $data['id'] . '" method="post" enctype="multipart/form-data">                    
            ...
            $row = [
                ...
                'tgl_raport_pts' => $data['tgl_raport_pts'],
                'tgl_raport_pas' => $data['tgl_raport_pas'],
                'tgl_raport_pat' => $data['tgl_raport_pat'],
    ...
    public function tambah()
    {
        ...
        $data['pegawai'] = $this->db->get_where('pegawai', ['email_pegawai' => $this->session->userdata('email_pegawai')])->row_array();
        $this->load->view('data_persiapan/persiapan_tahun_aktif/tahun_aktif_css');
        $this->load->view('data_persiapan/persiapan_tahun_aktif/tahun_aktif_tambah', $data);
        $this->load->view('data_persiapan/persiapan_tahun_aktif/tahun_aktif_js');
    }
    ...
    public function aktif()
    {
        ...
        ');
        redirect('data_persiapan/persiapan_tahun_aktif');
    }
    ...
    public function edit($id)
    {
        if ($this->form_validation->run() == false) {
            ...
            $this->load->view('layout/header-bottom', $data);
            $this->load->view('layout/main-navigation', $data);
            $this->load->view('data_persiapan/persiapan_tahun_aktif/tahun_aktif_edit', $data);
            $this->load->view('layout/footer-top');
            $this->load->view('data_persiapan/persiapan_tahun_aktif/tahun_aktif_js');
            $this->load->view('layout/footer-bottom');
            $this->benchmark->mark('code_end');
